...
float4 force_ij(float4 pos_i, float4 pos_j, float4 c1, float4 c2)
{
	...
	return float4(R.xyz * (r * (c1.x * r2 - c1.z * c1.y * exp(c1.z / r)) - 6 * c1.w * (r2 * r2) * (r2 * r2)), 0);
}
float4 energy_ij(float4 pos_i, float4 pos_j, float4 c1, float4 c2)
{
	...
	{
		R.w = rsqrt(R.w);
		float r3 = R.w * R.w * R.w, e = c1.y * exp(c1.z / R.w), cc = c1.w * r3 * r3;
		R.xyz = R.xyz * (c1.x * r3 - (c1.z * e + 6 * cc * R.w) * R.w);
		R.w = (c1.x * R.w + e - cc);
		return R;
	}
}
